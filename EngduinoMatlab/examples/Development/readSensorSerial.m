...
while (1)
        ...
        light=strfind(strSplitted,'Light value: ');
        accelerometer=strfind(strSplitted,'Accelerometer value: ');

        if(not(isempty(light{1})))
            str = strjoin(strSplitted(1));
            strSplitted2 = strsplit(str,{'Light value: '},'DelimiterType','RegularExpression');
            temp = str2double(strSplitted2{2});
            disp(temp);
        end
        if(not(isempty(accelerometer{1})))
            str = strjoin(strSplitted(1));
            strSplitted2 = strsplit(str,{'Accelerometer value: '},'DelimiterType','RegularExpression');
            temp = str2double(strSplitted2{2});
            disp(temp);
        end
