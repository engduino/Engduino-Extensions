classdef engduino < handle
    ...
    properties (Hidden=false)
        ...
        % Function to be called for every sample, must take arguments (temp, accel, mag_field, light, samples).
    ...
    properties (Hidden = true, SetAccess = protected, GetAccess = protected)
        %> Internal.
        leds_r_ = ones(1, engduino3.LED_COUNT).*engduino.LED_DEFAULT_BRIGHTNESS;
        %> Internal.
        leds_g_ = ones(1, engduino3.LED_COUNT).*engduino.LED_DEFAULT_BRIGHTNESS;
        %> Internal.
        leds_b_ = ones(1, engduino3.LED_COUNT).*engduino.LED_DEFAULT_BRIGHTNESS;
    end
            ...
            switch e.comm
                case e.COMM_COMPORT
                    ...
                    if (nargin == 0) % Get all available ports
                        %instr = instrhwinfo('serial');
                        %ports = instr.AvailableSerialPorts;
...
%                           disp('Serial port and other Methods: <a href="matlab:help serial">serial</a> <a href="matlab:help flush">flush</a> <a href="matlab:help roundTrip">roundTrip</a>');
    ...
    methods (Hidden=false) % internal methods
        ...
        function res = setStatus(e, vals)
            ...
            % Check parameters
            if (mod(length(vals), 2) ~= 0) % Number of input parameters must be even.
                e.showError('setStatus ERROR: Wrong number of input values! Size of values must be even.');
                res = e.RES_ERR;
                return;
            end
